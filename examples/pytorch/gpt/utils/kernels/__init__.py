...
def compress_int4_weight(weight: torch.Tensor):  # (n, m)
    with torch.cuda.device(weight.device):
        ...
        out = torch.zeros(n, m, dtype=torch.int8, device="cuda").contiguous()
        ...
        kernels.int4WeightCompression(
            ...
            0,
            stream,
            [ctypes.c_void_p(weight.contiguous().data_ptr()), ctypes.c_void_p(out.contiguous().data_ptr()), ctypes.c_int32(n), ctypes.c_int32(m)],
        )
        return out
