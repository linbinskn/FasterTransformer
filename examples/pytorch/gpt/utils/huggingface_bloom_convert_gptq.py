...
def get_args():
    ...
    parser.add_argument(
        '-ik', '--input-ckpt', type=str, metavar='DIR', required=True,
        help='A checkpoint directory of a huggingface pretrained model.')
    parser.add_argument(
        '-ic', '--input-config', type=str, metavar='DIR', required=True,
        help='A config of a huggingface pretrained model.')
    parser.add_argument(
    ...
    parser.add_argument(
        '-dt', '--data-type', type=str, metavar='STR', default='fp16',
...
parameter_rename_map = {
    ...
    # scale related
    'self_attention.dense.scale': 'attention.dense.scale',
    'self_attention.query_key_value.scale': 'attention.query_key_value.scale'
}
...
parameter_to_split = [
    ...
    # scale related
    ('attention.query_key_value.scale', -1),
    ('mlp.dense_h_to_4h.scale', -1)
]
...
def reorder_qkv_weight_or_bias(model_config: PretrainedConfig,
    ...
    head_dim = model_config.hidden_size // model_config.n_head if 'weight' not in name else (model_config.hidden_size // 2) // model_config.n_head
...
def main():
    ...
    config = model.config
    model_and_scale = torch.load(args.input_ckpt)
    model = model_and_scale['model']
    quant_factors = model_and_scale['quant_factors']

    for scale_name, scale_param in quant_factors.items():
        model['.'.join([scale_name, 'scale'])] = scale_param['scale']

    save_dir = Path(args.output_dir) / f'{tp_size}-gpu'
    save_dir.mkdir(exist_ok=True, parents=True)
    save_bloom_config(config, save_dir)

    start_time = time.time()
    logger.info(f'Start the checkpoint conversion: '
                f'{len(list(model.keys()))} params')
    if args.processes > 1:
